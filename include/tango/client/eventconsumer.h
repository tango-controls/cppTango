...
protected :
    ...
    static map<std::string, EventChannelStruct>
        channel_map;            // key - channel_name (full adm name), value - Event Channel info
    static map<std::string, EventCallBackStruct>
...
public :
    ...
    virtual void get_subscription_command_name(string &cmd)
...
public:
    DelayedEventUnsubThread(EventConsumer *ec, int id, TangoMonitor *m)
        : omni_thread(), event_id(id), ev_cons(ec), the_mon(m)
    {}
...
public:
    DelayedEventSubThread(EventConsumer *ec, DeviceProxy *_device,
                          ...
                          int _id)
        : omni_thread(), ev_cons(ec), device(_device),
