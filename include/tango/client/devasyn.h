...
public:
	...
	TgRequest(TangoRequest_ptr re,ReqType ty):request(re),req_type(ty),cb_ptr(NULL),
						    					arrived(false),dev(NULL)
	{};

	TgRequest(TangoRequest_ptr re,ReqType ty,CallBack *cb):request(re),req_type(ty),cb_ptr(cb),
							 								 arrived(false),dev(NULL)
	{};
...
public:
	...
	TgRequest &get_request(TangoRequest_ptr);
	TgRequest *get_request(Tango::Connection *);

	long store_request(TangoRequest_ptr,TgRequest::ReqType);
	void store_request(TangoRequest_ptr,CallBack *,Connection *,TgRequest::ReqType);

	void remove_request(long);
	void remove_request(Connection *,TangoRequest_ptr);
	...
	void mark_as_arrived(TangoRequest_ptr req);
...
protected:
	...
	map<TangoRequest_ptr,TgRequest>	cb_req_table;
