			     ...
			     long vers,DeviceAttribute *dev_attr)
{
	...
	const DevVarLong32Array *tmp_seq_lo32;
	const DevVarLongLongArray *tmp_seq_lolo;
#endif
...
#else
					attr_value_3->value >>= tmp_seq_lo32;
					max = tmp_seq_lo32->maximum();
					len = tmp_seq_lo32->length();
					if (tmp_seq_lo32->release() == true)
					{
						tmp_lo = (const_cast<DevVarLong32Array *>(tmp_seq_lo32))->get_buffer((CORBA::Boolean)true);
						long *tmp_lo64 = DevVarLongArray::allocbuf(len);
						for (unsigned int l = 0;l < len;l++)
							tmp_lo64[l] = tmp_lo[l];
						DevVarLong32Array::freebuf(tmp_lo);
						dev_attr->LongSeq = new DevVarLongArray(max,len,tmp_lo64,true);
					}
					else
					{
						tmp_lo = const_cast<CORBA::Long *>(tmp_seq_lo32->get_buffer());
						long *tmp_lo64 = DevVarLongArray::allocbuf(len);
						for (unsigned int l = 0;l < len;l++)
							tmp_lo64[l] = tmp_lo[l];
						dev_attr->LongSeq = new DevVarLongArray(max,len,tmp_lo64,true);
...
#endif
					dev_attr->ext->long_from_64 = false;
					break;
		
				case CORBA::tk_longlong:
#ifdef TANGO_LONG64
					if (vers >= 3)
						attr_value_3->value >>= tmp_seq_lolo;
					else		
						attr_value->value >>= tmp_seq_lolo;
					max = tmp_seq_lolo->maximum();
					len = tmp_seq_lolo->length();
					if (tmp_seq_lolo->release() == true)
					{
						tmp_lolo = (const_cast<DevVarLongLongArray *>(tmp_seq_lolo))->get_buffer((CORBA::Boolean)true);
						dev_attr->LongSeq = new DevVarLongArray(max,len,tmp_lolo,true);
					}
					else
					{
						tmp_lolo = const_cast<CORBA::LongLong *>(tmp_seq_lolo->get_buffer());
						dev_attr->LongSeq = new DevVarLongArray(max,len,tmp_lolo,false);
					}
#else
					dev_attr->ext->long_from_64 = true;
#endif
					break;
...
//
	...
	o_str << "Attribute alarm : min alarm = " << p.alarms.min_alarm << endl;	
	o_str << "Attribute alarm : max alarm = " << p.alarms.max_alarm << endl;	
	o_str << "Attribute warning alarm : min warning = " << p.alarms.min_warning << endl;	
	o_str << "Attribute warning alarm : max warning = " << p.alarms.max_warning << endl;	
	o_str << "Attribute rds alarm : delta time = " << p.alarms.delta_t << endl;	
	o_str << "Attribute rds alarm : delta value = " << p.alarms.delta_val << endl;	

	for (i = 0;i < p.alarms.extensions.size();i++)
		o_str << "Attribute alarm extensions " << i + 1 << " = " << p.alarms.extensions[i] << endl;


	o_str << "Attribute event : change event absolute change = " << p.events.ch_event.abs_change << endl;
	o_str << "Attribute event : change event relative change = " << p.events.ch_event.rel_change << endl;
	...
	o_str << "Attribute event : periodic event period = " << p.events.per_event.period << endl;
	...
	o_str << "Attribute event : archive event absolute change = " << p.events.arch_event.archive_abs_change << endl;
	o_str << "Attribute event : archive event relative change = " << p.events.arch_event.archive_rel_change << endl;
	o_str << "Attribute event : archive event period = " << p.events.arch_event.archive_period;
