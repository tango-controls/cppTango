...
//
// subscribe to a periodic event and a change event
//
...
//
// Now, kill device server
//

		cout << "Kill device server process and hit \"return\" key when done" << endl;
...
//
		...
		int old_cb_err = cb.cb_err;
		int old_cb_err_ch = cb_change.cb_err;

		wait_and_dot(25);
		
		int new_cb_err = cb.cb_err;
		int new_cb_err_ch = cb_change.cb_err;
...
//
// Now, restart device server
//

		int old_cb_no_err = cb_change.cb_no_err;
		cout << "Restart device server and hit \"return\" key when done" << endl;
		cout << "Process will wait for callback during 25 sec";
		getline(cin,key);
...
//
		...
		int new_cb_no_err = cb_change.cb_no_err;
