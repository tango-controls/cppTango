...
char* ToCORBAString(CORBA::ULong val)
{
...
CORBA::Any*   FileDatabase :: DbGetDeviceProperty(CORBA::Any& send)
{
	...
	(*data_out)[index] = ToCORBAString(num_prop); index++;
		...
		for(i = 0; i < nb_defined_dev; i++)
		{
			if ( equalsIgnoreCase((*data_in)[0].in(), m_server.devices[i]->name))
			{
              ...
              int num_val  = 0;
							...
							(*data_out)[index] = ToCORBAString(num_val); index++;
...
CORBA::Any*   FileDatabase :: DbGetDeviceAttributeProperty(CORBA::Any& send)
{
	...
	(*data_out)[index] = ToCORBAString(num_attr); index++;
	...
	if (dev_it != m_server.devices.end())
	{
		 ...
		 for(unsigned int k =0; k < num_attr; k++)
		 {
          ...
          int num_attr_find = 0;
					...
					(*data_out)[index-1] = ToCORBAString(num_prop);
            ...
            auto num_attr_prop = (*dev_it)->attribute_properties[j]->properties[l]->value.size();
						(*data_out)[index] = ToCORBAString(num_attr_prop); index++;
...
CORBA::Any*   FileDatabase :: DbGetClassAttributeProperty(CORBA::Any& send)
{
	...
	(*data_out)[0] = Tango::string_dup((*data_in)[0]); index++;
	auto num_attr = data_in->length() - 1;
	(*data_out)[1] = ToCORBAString(num_attr); index++;
...
CORBA::Any*  FileDatabase :: DbInfo(CORBA::Any&){
  ...
  auto generateString = [](std::string prefix, CORBA::ULong size) -> char*
  {
    prefix += std::to_string(size);
    return Tango::string_dup(prefix.c_str());
  };

  auto propFunc = [](CORBA::ULong init, auto s) -> CORBA::ULong
  {
    return init + s->properties.size();
  };

  auto propAttrFunc = [](CORBA::ULong init, auto s) -> CORBA::ULong
  {
    return init + s->attribute_properties.size();
  };

  auto accumulate = [](auto vec, auto func) -> CORBA::ULong
  {
  ...
  auto header = std::string("TANGO FileDatabase  ") + filename;
	...
	(*data_out)[4] = generateString("Devices defined = ", m_server.devices.size());
	(*data_out)[5] = generateString("Devices exported = ", m_server.devices.size());
	...
	(*data_out)[9] = generateString("Class properties defined = ", accumulate(m_server.classes, propFunc));
	(*data_out)[10] = generateString("Device properties defined = ", accumulate(m_server.devices, propFunc));
	(*data_out)[11] = generateString("Class attribute properties defined = ", accumulate(m_server.classes, propAttrFunc));
	(*data_out)[12] = generateString("Device attribute properties defined = ", accumulate(m_server.devices, propAttrFunc));
