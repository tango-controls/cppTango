...
#ifdef TANGO_HAS_LOG4TANGO
              logger(NULL), saved_log_level(log4tango::Level::WARN), rft(Tango::kDefaultRollingThreshold),
              poll_old_factor(0), idl_version(1),
#endif
...
//--------------------------------------------------------------------------
    ...
    //TODO remove code duplication: rem_obj; exit_cmd etc
...
//
            ...
            for (i = 0; i < nb_updated_attr; i++) {
                try {
                    dev_attr->get_w_attr_by_ind(updated_attr[i]).check_written_value(values[i].value, (unsigned long) 1,
                                                                                     (unsigned long) 0);
                }
                catch (Tango::DevFailed &) {
                    for (long j = 0; j < i; j++)
        ...
        switch (att.get_data_type()) {
            ...
            case Tango::DEV_ENCODED : {
                if (aid.data_3 != Tango_nullptr) {
                    ...
                    (*aid.data_3)[index].err_list[0].origin = Tango::string_dup(
                    ...
                    if (aid.data_5 != Tango_nullptr) {
                        ...
                        if (ptr->release() == true) {
                            ...
                            the_seq[0].encoded_data.replace(nb_data, nb_data, (*ptr)[0].encoded_data.get_buffer(true),
                                                            true);
            ...
            for (size_t i = 0; i < nb_att; ++i) {
                ...
                switch (fwd_att_wrong_conf[i].fae) {
                    ...
                    case FWD_ROOT_DEV_NOT_STARTED:
                        alarm_status = alarm_status +
                                       "Root device not started yet. Polling (if any) for this attribute not available";
                        break;
                    ...
                    case FWD_DOUBLE_USED: {
                        alarm_status = alarm_status +
                                       "Root attribute already used in this device server process for attribute ";
...
//
            ...
            if (still_default == true) {
                try {
                    vector<PipeProperty> &cl_pi_prop = device_class->get_class_pipe()->get_prop_list(
                            pi_ptr->get_name());
