...
//--------------------------------------------------------------------------------------------------------------------
    ...
    void DServer::event_subscription(string &dev_name, string &obj_name, string &action, string &event,
...
//
            ...
            cout4 << "ZmqEventSubscriptionChangeCmd: subscription for device " << dev_name << " attribute/pipe "
...
//
            ...
            event_subscription(dev_name, obj_name, action, event, obj_name_lower, ZMQ, mcast, rate, ivl, dev,
                               client_release);
...
//
            ...
            if (mcast.empty() == false) {
                ...
                if ((c_addr->client_ip[5] == 'u') ||
                    ((c_addr->client_ip[9] == '1') && (c_addr->client_ip[10] == '2') &&
                     (c_addr->client_ip[11] == '7'))) {
                    local_call = true;
                }
