...
# endif
    ...
    Util::Util(int argc, char *argv[]) : cl_list_ptr(NULL), ext(new UtilExt),
                                         heartbeat_th(NULL), heartbeat_th_id(0), poll_mon("utils_poll"), poll_on(false),
                                         ser_model(BY_DEVICE),
                                         only_one("process"), py_interp(NULL), py_ds(false), py_dbg(false),
                                         db_cache(NULL), inter(NULL), svr_starting(true), svr_stopping(false),
                                         poll_pool_size(ULONG_MAX),
                                         conf_needs_db_upd(false), ev_loop_func(NULL), shutdown_server(false),
                                         _dummy_thread(false),
                                         zmq_event_supplier(NULL), endpoint_specified(false), user_pub_hwm(-1),
...
//
            ...
            options.emplace("clientCallTimeOutPeriod", CLNT_TIMEOUT_STR);
            options.emplace("serverCallTimeOutPeriod", "5000");
            options.emplace("maxServerThreadPoolSize", "100");
            options.emplace("threadPerConnectionUpperLimit", "55");
            options.emplace("threadPerConnectionLowerLimit", "50");
            options.emplace("supportCurrent", "0");
            options.emplace("verifyObjectExistsAndType", "0");
            options.emplace("maxGIOPConnectionPerServer", MAX_GIOP_PER_SERVER);
            options.emplace("giopMaxMsgSize", MAX_TRANSFER_SIZE);
#ifndef _TG_WINDOWS_
            options.emplace("endPoint", "giop:unix:");
#endif
            options.emplace("throwTransientOnTimeOut", "1");

            if (get_endpoint_specified() == true)
                options.emplace("endPointPublish", "all(addr)");


            orb = TangORB_init(argc, argv, options);
            ...
            if (pos == string::npos) {
                ...
                for (size_t i = 0; i < ip_list.size() && !host_found; i++) {
                    if (getaddrinfo(ip_list[i].c_str(), NULL, &hints, &info) == 0) {
                        ptr = info;
                        while (ptr != NULL) {
                            if (getnameinfo(ptr->ai_addr, ptr->ai_addrlen, tmp_host, NI_MAXHOST, NULL, 0,
                                            NI_NAMEREQD) ==
                                0) {
