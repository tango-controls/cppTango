        ...
        if (env_var_fqdn_prefix.empty() == true) {
            try {
                ...
                for (size_t loop = 0; loop < env_var_fqdn_prefix.size(); ++loop)
                    transform(env_var_fqdn_prefix[loop].begin(), env_var_fqdn_prefix[loop].end(),
...
//-------------------------------------------------------------------------------------------------------------------
    ...
    int EventConsumer::subscribe_event(DeviceProxy *device,
        ...
        if ((device == NULL) || (event != INTERFACE_CHANGE_EVENT)) {
            EventSystemExcept::throw_exception(API_InvalidArgs,
                                               "Device NULL or unsupported event type",
                                               "EventConsumer::subscribe_event()");
        }
...
//
        ...
        pair<EvCbIte, bool> ret = event_callback_map.insert(
...
//

        {
            ...
            for (epos = event_callback_map.begin(); epos != event_callback_map.end(); ++epos) {
                for (esspos = epos->second.callback_list.begin();
                     esspos != epos->second.callback_list.end(); ++esspos) {
...
//
                        ...
                        if (this_thread::get_id() == thread_id) {
                            ...
                            std::thread(&DelayedEventUnsubThread::run,new DelayedEventUnsubThread(this, event_id, epos->second.callback_monitor)).detach();
...
//
                    ...
                    if (evt_cb.callback_list.empty() == true) {
                        ...
                        if (evt_ch.channel_type == NOTIFD) {
                            try {
                                CosNotifyFilter::Filter_var f = evt_ch.structuredProxyPushSupplier->get_filter(
                                        evt_cb.filter_id);
                                    ...
                                    if (evt_ch.adm_device_proxy != NULL) {
                                        if (evt_ch.channel_type == NOTIFD) {
                                            try {
                                                ...
                                                f->destroy();
                                                try {
                                                    omniORB::setClientCallTimeout(evt_ch.structuredProxyPushSupplier,
                                                                                  1000);
...
//-------------------------------------------------------------------------------------------------------------------
    ...
    void
    EventConsumer::get_fire_sync_event(DeviceProxy *device, CallBack *callback, EventQueue *ev_queue, EventType event,
...
//--------------------------------------------------------------------------
    ...
    FwdEventData::FwdEventData(DeviceProxy *dev, string &_s1, string &_s2, Tango::DeviceAttribute *_da,
                               DevErrorList &_del) :
            EventData(dev, _s1, _s2, _da, _del), av_5(Tango_nullptr), event_data(Tango_nullptr) {
    }

    FwdEventData::FwdEventData(DeviceProxy *dev, string &_s1, string &_s2, Tango::DeviceAttribute *_da,
                               DevErrorList &_del, zmq::message_t *_m) :
