...
void PollThread::print_list()
{
	...
	for (i = 0;i < nb_elt;i++)
	{
		cout << "Dev name = " << ite->dev->get_name();
		cout << ", obj name = " << ite->name;
		cout << ", next wake_up at " << + ite->wake_up_date.tv_sec;
		cout << "," << setw(6) << setfill('0') << ite->wake_up_date.tv_usec << endl;
		ite++;
	}
}
...
void PollThread::poll_cmd(WorkItem &to_do)
{
	...
	cout5 << setw(6) << setfill('0') << now.tv_usec;
	cout5 << " Dev name = " << to_do.dev->get_name();
	cout5 << ", Cmd name = " << to_do.name << endl;
...
void PollThread::poll_attr(WorkItem &to_do)
{
	...
	cout5 << setw(6) << setfill('0') << now.tv_usec;
	cout5 << " Dev name = " << to_do.dev->get_name();
	cout5 << ", Attr name = " << to_do.name << endl;
