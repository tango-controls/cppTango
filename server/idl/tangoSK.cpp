...
void
Tango::DevVarLong32StringArray::operator>>= (cdrStream &_n) const
{
  (const _CORBA_Unbounded_Sequence_w_FixSizeElement< CORBA::Long, 4, 4 > &) lvalue >>= _n;
  (const DevVarStringArray&) svalue >>= _n;

}

void
Tango::DevVarLong32StringArray::operator<<= (cdrStream &_n)
{
  (_CORBA_Unbounded_Sequence_w_FixSizeElement< CORBA::Long, 4, 4 > &)lvalue <<= _n;
  (DevVarStringArray&)svalue <<= _n;
...
// Local call call-back function.
static void
_0RL_lcfn_6FE2F94A21A10053_b0000000(omniCallDescriptor*, omniServant* svnt)
{
...
void Tango::Device_3_Helper::duplicate(::Tango::Device_3_ptr p) {
  if( p && !p->_NP_is_nil() )  omni::duplicateObjRef(p);
}
