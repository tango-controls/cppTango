...
/**
 * Fire a change event for the attribute value. The attibute data must be
 * set with one of the Attribute::set_value or Attribute::setvalue_date_quality methods 
 * before.
 * Push a change event to the Notification service daemon.
 * A change event is always fired when a change of the quality is detected.
 *
 * @param except A pointer to a DevFailed exception to be thrown as change event.
 */	
...
/**
 * Fire a archive event for the attribute value. The attibute data must be
 * set with one of the Attribute::set_value or Attribute::setvalue_date_quality methods 
 * before.
 * Push a archive event to the Notification service daemon.
 *
...
//
	...
	void set_change_event_sub() {ext->event_change_subscription=(long)time(NULL);}
	void set_periodic_event_sub() {ext->event_periodic_subscription=(long)time(NULL);}
	void set_archive_event_sub() {ext->event_archive_subscription=(long)time(NULL);}
	void set_quality_event_sub() {ext->event_quality_subscription=(long)time(NULL);}
	void set_user_event_sub() {ext->event_user_subscription=(long)time(NULL);}		
...
//@{
/**
 * A flag set to true if a manual fire change event is implemented.
 */
	bool			change_event_implmented;
/**
 * A flag set to true if a manual fire archive event is implemented.
 */
	bool			archive_event_implmented;	
/**
 * A flag set to true if the change event criteria should be checked when sending
 * the event.
 */
	bool			check_change_event_criteria;
/**
 * A flag set to true if the archive event criteria should be checked when sending
 * the event.
 */
	bool			check_archive_event_criteria;			
