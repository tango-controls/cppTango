 ...
 *
 * A change event is always fired a change of the quality is detected.
 *
 * @param detect Triggers the verification of the change event properties when set to true.
 * @param except A pointer to a DevFailed exception to be thrown a change event.
 */	
	void fire_change_event(bool detect = true, DevFailed *except = NULL);

/**
 * Set a flag to indicate that the server fires change events manually without
 * the polling to be started for the attribute
 *
 * @param impemented	True when the server fires change events manually. 
 */
	void set_change_event(bool implemented) {change_event_implmented = implemented;}	
