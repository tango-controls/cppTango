...
//
			...
			for (i = 0;i < nb_wanted_attr;i++)
			{
				...
				catch (Tango::DevFailed)
				{
					for (j = 0;j < i;j++)
					{
						dev_attr->get_attr_by_ind(attr_list[j]).delete_seq();
					}
...
// set to ALARM. It is not necessary to do this if we have already detected
...
//
	...
	for (i = 0;i < nb_names;i++)
	{
		...
		if ((qual == Tango::ATTR_VALID) || (qual == Tango::ATTR_ALARM) || (qual == Tango::ATTR_CHANGING))
		{
