...
:Attribute(prop_list,tmp_attr,dev_name,idx),memorized(false),w_ext(NULL)
{
...
void WAttribute::set_value()
{
	...
	case Tango::DEV_SHORT:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&short_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevShort *>(short_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
		
	case Tango::DEV_LONG:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&long_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevLong *>(long_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
				
	case Tango::DEV_DOUBLE:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&double_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevDouble *>(double_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
		
	case Tango::DEV_STRING:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&str_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevString *>(str_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
		
	case Tango::DEV_FLOAT:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&float_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevFloat *>(float_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
		
	case Tango::DEV_BOOLEAN:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&boolean_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevBoolean *>(boolean_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
				
	case Tango::DEV_USHORT:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&ushort_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevUShort *>(ushort_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
		
	case Tango::DEV_UCHAR:
		if (data_format == Tango::SCALAR)
			Attribute::set_value(&uchar_val,1,0,false);
		else
			Attribute::set_value(const_cast<DevUChar *>(uchar_array_val.get_buffer()),w_dim_x,w_dim_y,false);
		break;
	}
}
...
//
		...
		case Tango::DEV_FLOAT:
			...
			for (i = 0;i < nb_data;i++)
			{
				float delta = double_array_val[i] - (*value.db_seq)[i];
#if ((defined __SUNPRO_CC) || (defined WIN32))
				if (((float)fabs(delta)) >= delta_val.fl)
#else
