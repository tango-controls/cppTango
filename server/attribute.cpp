...
//
// Compute data size and set default quality to valid only if the attribute
// quality is not already set to CHANGING and if a client has subscribed to
// a quality change event on this attribute !!
// All this quality factor management for scannable device.
//

	dim_x = x;
	dim_y = y;
	set_data_size();
	if (quality != Tango::ATTR_CHANGING)
		quality = Tango::ATTR_VALID;
	else
	{
		bool quality_subscribed = false;
					   ...
					   Tango::DeviceImpl *dev)
{
	if (quality != Tango::ATTR_INVALID)
	{
		...
		case Tango::DEV_SHORT :
			...
			sh_seq = new Tango::DevVarShortArray(data_size,data_size,sh_tmp_ptr,false);
			a <<= sh_seq;
			break;

		case Tango::DEV_LONG :
			lo_tmp_ptr = get_long_value()->get_buffer();
			lo_seq = new Tango::DevVarLongArray(data_size,data_size,lo_tmp_ptr,false);
			a <<= lo_seq;
			break;
		...
		case Tango::DEV_STRING :
			...
			str_seq = new Tango::DevVarStringArray(data_size,data_size,str_tmp_ptr,false);
			a <<= str_seq;
			break;

		case Tango::DEV_FLOAT : 		
			fl_tmp_ptr = get_float_value()->get_buffer();
			fl_seq = new Tango::DevVarFloatArray(data_size,data_size,fl_tmp_ptr,false);
			a <<= fl_seq;
			break;

		case Tango::DEV_BOOLEAN :
			bo_tmp_ptr = get_boolean_value()->get_buffer();
			bo_seq = new Tango::DevVarBooleanArray(data_size,data_size,bo_tmp_ptr,false);
			a <<= bo_seq;
			break;

		case Tango::DEV_USHORT :
			ush_tmp_ptr = get_ushort_value()->get_buffer();
			ush_seq = new Tango::DevVarUShortArray(data_size,data_size,ush_tmp_ptr,false);
			a <<= ush_seq;
			break;

		case Tango::DEV_UCHAR :
			uch_tmp_ptr = get_uchar_value()->get_buffer();
			uch_seq = new Tango::DevVarUCharArray(data_size,data_size,uch_tmp_ptr,false);
			a <<= uch_seq;
			break;
		}
