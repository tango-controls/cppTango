...
Device_2Impl(device_class,dev_name),ext_3(NULL)
{
	ext->idl_version = 3;
...
Device_3Impl::Device_3Impl(DeviceClass *device_class,
			   string &dev_name,
			   string &desc):
Device_2Impl(device_class,dev_name,desc),ext_3(NULL)
{
	ext->idl_version = 3;
	add_state_status_attrs();
}
...
//				read_attr(dev_attr->get_attr_by_ind(wanted_attr[i].idx_in_multi_attr));
			}
			catch (Tango::DevFailed &e)
			{
				long index;
	...
	for (i = 0;i < nb_wanted_w_attr;i++)
	{
		try
		{
			...
			if ((w_type == Tango::READ_WITH_WRITE) ||
		    	    (w_type == Tango::WRITE)) 
				dev_attr->get_attr_by_ind(wanted_w_attr[i].idx_in_multi_attr).set_value();
		}
		catch (Tango::DevFailed &e)
		{
			long index;
