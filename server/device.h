...
//
// WARNING : If you add a new data member, don't forget to also add it in the
...
//
	DeviceImplExt():exported(false),polled(false),poll_ring_depth(0)
			only_one("dev"),store_in_bb(true),poll_mon("cache"),
 ...
 * Push a user event
 *
 * Push a user event to the Notification service daemon.
 * This method consumes some memory used to store attribute value. If the
 * attribute value stored into the Attribute object is still needed after
 * this method has been called, it needs to be re-set with one of
 * the Attribute::set_value method.
 *
 * @param filterable_name The object type (command or attribute)
 * @param filterable_value The object name
 * @param att The attribute object
 * @param err The error stack (if any). A default value is provided (NULL)
 * @exception DevFailed Thrown if the object is not found.
 * Click <a href="../../../tango_idl/idl_html/Tango.html#DevFailed">here</a> to read
 * <b>DevFailed</b> exception specification
 */	
	void push_event(vector<string> &filterable_name,vector<double> &filterable_value,Tango::Attribute &att,Tango::DevErrorList *err=NULL);
//@}
...
#ifdef TANGO_HAS_LOG4TANGO
 	inline log4tango::Logger *get_logger(void) const
...
#ifdef TANGO_HAS_LOG4TANGO
  	log4tango::Logger *get_logger_i (void) const;
#endif
