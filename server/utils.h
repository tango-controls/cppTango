...
{
public :
	TangoMonitor():cond(this),used(false),locking_thread(NULL),
		       locked_ctr(0),_timeout(DEFAULT_TIMEOUT) {};
	~TangoMonitor() {};
...
{
public:
	AutoTangoMonitor(TangoMonitor &moni):mon(moni) {mon.get_monitor();}
	~AutoTangoMonitor() {mon.rel_monitor();}
...
{
public:
	UtilExt() {shared_data.cmd_pending = false;}
...
inline void TangoMonitor::get_monitor()
{
	...
	cout4 << "In get_monitor(), used = " << used << ", thread = " << hex << omni_thread::self() << dec << ", ctr = " << locked_ctr << endl;
//	cout4 << "In get_monitor(), used = " << used << endl;
...
cout4 << "waiting !!" << endl;
		...
		if ((used == true) && 
		    (interupted == false))
		{			
			cout4 << "TIME OUT for thread " << hex << omni_thread::self() << dec << endl;
			Except::throw_exception((const char *)"API_CommandTimedOut",
					        (const char *)"Not able to acquire device monitor",
					        (const char *)"TgMonitor::get_monitor");
		}
	}
	locked_ctr++;
	if (!owner_thread)
...
inline void TangoMonitor::rel_monitor()
{
	...
	cout4 << "In rel_monitor(), used = " << used << ", ctr = " << locked_ctr << ", thread = " << hex << locking_thread << dec << endl;
//	cout4 << "In rel_monitor(), used = " << used << ", ctr = " << locked_ctr << endl;
