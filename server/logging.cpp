...
log4tango::Logger* Logging::_core_logger = 0;
...
{
  if (Logging::_core_logger) {
    delete Logging::_core_logger;
    Logging::_core_logger = 0;
  }
}
  ...
  try {
    ...
    for (i = 0; i < argin->length();)
    {
      ...
      for (unsigned int j = 0; j < dl.size(); j++) {
        ...
        if (logger == 0) {
          TangoSys_OMemStream o;
          o << "Internal error (got a NULL looger)" << ends;
          Except::throw_exception((const char *)"API_InternalError", o.str(),
                                  (const char *)"Logging::remove_logging_target");
        } 
