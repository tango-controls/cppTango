...
//
// If it is writable, remove it to the writable attribute list
//

	Tango::AttrWriteType w_type = attr_list[att_index]->get_writable();
	if ((w_type == Tango::WRITE) ||
	    (w_type == Tango::READ_WRITE))
	{
		vector<long>::iterator pos;
		
		pos = remove(writable_attr_list.begin(),
		       	     writable_attr_list.end(),
		             att_index);
		writable_attr_list.erase(pos,writable_attr_list.end());
	}
		
//
// If one of the alarm properties is defined, remove it to the alarmed attribute
// list
//

	if (attr_list[att_index]->is_alarmed().any() == true)
	{
		if (w_type != Tango::WRITE)
		{
			vector<long>::iterator pos;

			pos = remove(alarm_attr_list.begin(),
		       		     alarm_attr_list.end(),
		        	     att_index);
			alarm_attr_list.erase(pos,writable_attr_list.end());
		}
	}
