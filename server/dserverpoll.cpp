...
//
	...
	if (poll_th_id == 0)
	{
		cout4 << "Creating a thread to poll device " << (argin->svalue)[0] << endl;
		thread_created = tg->create_poll_thread((argin->svalue)[0],false);
		poll_th_id = tg->get_polling_thread_id_by_name((argin->svalue)[0]);
	}

	cout4 << "Thread in charge of device " << (argin->svalue)[0] << " is thread " << poll_th_id << endl;
...
//
		...
		if (kill_thread == true)
		{
			...
			void *dummy_ptr;
			cout4 << "Joining with one polling thread" << endl;
